...
const PageLoader = () => (
  <div className="flex items-center justify-center min-h-[50vh]">
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="text-sm text-gray-400 font-medium">Loading quotations and products…</p>
    </div>
  </div>
);
...
const FormFields = ({
  ...
  modalLastAddedProduct, setModalLastAddedProduct, modalSubmitLoading,
}) => (
  <div className="space-y-5">
    ...
    <div className="flex justify-end gap-2 pt-1">
      <button
        ...
        className="px-4 py-2 rounded-lg border border-gray-200 bg-white text-gray-600 text-sm font-semibold hover:bg-gray-50 transition-colors disabled:opacity-50"
      >
        Cancel
      </button>
      <button
        onClick={handleSubmit}
        disabled={!modalSelectedCustomer || !modalCart.length || modalSubmitLoading}
        className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold text-white transition-all duration-150
          ${!modalSelectedCustomer || !modalCart.length || modalSubmitLoading
            ? "bg-gray-200 text-gray-400 cursor-not-allowed"
            : isEdit
              ? "bg-amber-500 hover:bg-amber-600 shadow-sm"
              : "bg-blue-600 hover:bg-blue-700 shadow-sm"}`}
      >
        {modalSubmitLoading && <Spinner size="sm" color="text-white" />}
        {modalSubmitLoading
...
const CancelConfirmModal = ({ isOpen, onClose, onConfirm, quotationId, cancelLoading }) => (
  <Modal isOpen={isOpen} onRequestClose={onClose} className="fixed inset-0 flex items-center justify-center p-4 z-50" overlayClassName="fixed inset-0 bg-black/40 backdrop-blur-sm z-40">
    <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full overflow-hidden">
      <div className="p-6 text-center">
        ...
        <div className="flex gap-2 justify-center">
          <button onClick={onClose} disabled={cancelLoading} className="flex-1 max-w-[140px] py-2 rounded-lg border border-gray-200 bg-white text-gray-600 text-sm font-semibold hover:bg-gray-50 transition-colors disabled:opacity-50">
            Keep It
          </button>
          <button onClick={onConfirm} disabled={cancelLoading} className="flex-1 max-w-[140px] py-2 rounded-lg bg-red-600 text-white text-sm font-bold hover:bg-red-700 transition-colors shadow-sm flex items-center justify-center gap-1.5 disabled:opacity-50">
            {cancelLoading && <Spinner size="sm" color="text-white" />}
...
export default function Direct() {
  ...
  const exportQuotationsToExcel = async () => {
    ...
    try {
      ...
      const filename = match?.[1] || `DeepaCrackers_Export_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  return (
    <DirectErrorBoundary>
      <div className="flex min-h-screen bg-[#f5f6f8]">
        ...
        <div className="hundred:ml-64 mobile:ml-0 flex-1 hundred:px-8 mobile:px-4 pt-8 pb-16">
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
              ...
              <button onClick={exportQuotationsToExcel} disabled={exportLoading} className="flex items-center gap-2 h-9 px-4 rounded-lg border border-gray-200 bg-white text-xs font-semibold text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-all disabled:opacity-60">
                {exportLoading ? <Spinner size="sm" color="text-violet-500" /> : <FaDownload className="text-violet-500" />}
                {exportLoading ? "Exporting…" : "Export Quotations"}
              </button>
            </div>
            </>
            )}
        ...
        <CancelConfirmModal isOpen={cancelConfirmOpen} onClose={() => setCancelConfirmOpen(false)} onConfirm={cancelQuotation} quotationId={quotationToCancel} cancelLoading={cancelLoading} />
